...
# MAGIC %md
# MAGIC ## ADVANCED CHATBOT WITH MESSAGE HISTORY AND FILTER USING LANGCHAIN AND DBRX INSTRUCT
...
rag_chain_config = {
    ...
    "input_example": {
        "messages": [
            {"role": "user", "content": "What does Sophinea do?"},
            {
                "role": "assistant",
                "content": "Sophinea provides Business Intelligence and Data Analytics Services to protect the national interests of the United States and advance peace, security, and sustainable development.",
            },
    ...
    "retriever_config": {
        ...
        "vector_search_index": f"{catalog}.{db}.sophinea_self_managed_vs_index", #variable
    },
}
try:
...
with mlflow.start_run(run_name=f"sophinea_dbdemos_rag_advanced"):
...
MODEL_NAME = "sophinea_dbdemos_rag_advanced"
MODEL_NAME_FQN = f"{catalog}.{db}.{MODEL_NAME}"

# COMMAND ----------

# MAGIC %md
# MAGIC ## GPU NOT POSSIBLE?
...
instructions_to_reviewer = f"""### Instructions for Testing the our Sophinea RFP Chatbot Assistant
...
user_list = ["derek.satterley@databricks.com"]
# Set the permissions.
...
# MAGIC
# MAGIC ## NEXT: OFFLINE MODEL EVALUATION WITH MOSAIC AI AGENT EVALUATION
# MAGIC
