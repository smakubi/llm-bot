...
def custom_predict(endpoint, inputs):
    deploy_client = get_deploy_client("databricks")
    response = deploy_client.predict(endpoint=endpoint, inputs=inputs)
    return response['predictions']

class ModifiedDatabricksEmbeddings(DatabricksEmbeddings):
    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        return custom_predict(self.endpoint, {"inputs": texts})

    def embed_query(self, text: str) -> List[float]:
        return custom_predict(self.endpoint, {"inputs": [text]})[0]
#FIX
...
embedding_model = ModifiedDatabricksEmbeddings(endpoint=retriever_config.get("embedding_model"))
